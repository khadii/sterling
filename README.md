...
### Email confirmation: link and OTP

Email/password signups keep the existing Supabase confirmation link. They can also be confirmed with the six-digit OTP generated by that same Supabase signup event; Sterling does not generate, store, or manually confirm OTPs.

In **Supabase Dashboard → Authentication → Email Templates → Confirm signup**, retain the link and add the OTP to the same template:

```html
<p><a href="{{ .ConfirmationURL }}">Confirm email address</a></p>
<p>Or enter this verification code in the app: <strong>{{ .Token }}</strong></p>
```

Either method is single-use: successfully using the link or the code consumes the Supabase token, so the other method will then be invalid. `POST /auth/resend-confirmation` continues to resend this same dual-option confirmation email.

To confirm with a code, call the API rather than handling Supabase credentials separately in the frontend:

```ts
const response = await fetch(`${import.meta.env.VITE_API_URL}/auth/confirm-email-otp`, {
  method: 'POST',
...
if (!response.ok) {
  // API errors use the normal { statusCode, message, ... } contract.
  throw new Error(Array.isArray(body.message) ? body.message.join(', ') : body.message);
}

const { session } = body;
// Store session.access_token and session.refresh_token using the same policy as sign-in.
```

The route returns `200` with Supabase's `{ user, session }` response after a successful confirmation. It returns the standard API error body with `400` for a malformed, invalid, expired, or already-used code, and `429` when its request limit is exceeded. The route calls Supabase `verifyOtp({ email, token, type: 'email' })`, the documented verification type for email OTPs. Do not call `admin.updateUserById({ email_confirm: true })`; that would bypass proof of ownership.
...
| POST   | `/api/v1/auth/confirm-email-otp`   |    200 | Confirm a signup email with its Supabase six-digit OTP          |
